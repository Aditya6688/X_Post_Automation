...
# ==========================================
# MODULE 3: THE ENGINE (Updated for 1 Draft/Day)
# ==========================================

def run_ghostwriter_v2():
    print("--- 🎲 Rolling the Dice for Content Source ---")
    
    # 1. Randomly select ONE source type (GitHub, News, Papers, etc.)
    sources = [fetch_github_trending, fetch_hacker_news, fetch_arxiv_ai, fetch_dev_to]
    weights = [0.4, 0.3, 0.2, 0.1] # 40% chance for Code, 30% News...
    ...
    # 3. THE CHANGE: Pick only ONE item from the 3 fetched
    winner_item = random.choice(items)
    
    print(f"✅ Selected Winner: {winner_item['title']}")
    print("🧠 Generating draft...")

    # 4. Generate & Save ONLY that one item
    draft = generate_tweet(winner_item, winner_item['type'])
    
    try:
        notion.pages.create(
            parent={"database_id": NOTION_DB_ID},
            properties={
                "Name": {"title": [{"text": {"content": winner_item['title']}}]},
                "Tweet Draft": {"rich_text": [{"text": {"content": draft}}]},
                "Source URL": {"url": winner_item['link']},
                "Status": {"status": {"name": "Not started"}},
                "Type": {"select": {"name": winner_item['type']}} 
            }
        )
        print(f"🚀 Success! 1 Draft saved to Notion.")
